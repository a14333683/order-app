  ...
  return (
    ...
    >
      {items.map((label, index) => (
        <MenuItem key={index} i={index} label={label} />
      ))}
    </motion.ul>
  )
}

const MenuItem = ({ i, label }) => {
  const border = `2px solid ${colors[i % colors.length]}`import { motion } from "framer-motion"
  import { Link } from "react-router-dom"
  ...
  return (
    ...
    >
      <div style={{ ...iconPlaceholder, border }} />
      <Link to={path} style={{ ...textPlaceholder, border, textDecoration: "none" }}>
        {label}
      </Link>
    </motion.li>
  )
